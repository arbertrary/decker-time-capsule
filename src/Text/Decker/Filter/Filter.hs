...
promoteVerticalSlideHeader :: Slide -> Decker Slide
promoteVerticalSlideHeader (Slide (Just (Header 1 (id, cls, attr) text)) body)
  | "sub" `elem` cls =
    return (Slide (Just $ Header 2 (id, cls, attr) text) body)
promoteVerticalSlideHeader slide = return slide
...
wrapBoxes slide@(Slide header body) = do
  disp <- gets disposition
  case disp of
    Disposition Deck Html -> return $ Slide header $ concatMap wrap  boxes
  ...
  where
    ...
    wrap ((Header 2 (id_, cls, kvs) text) : blocks) =
      [ Div
          ("", "box" : cls, kvs)
          (Div (id_, "h2" : deFragment cls, kvs) [(Plain text)] : blocks)
      ]
    wrap box = box
