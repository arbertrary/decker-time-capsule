...
import qualified Text.Decker.Filter.Attributes as N
...
metaFlag :: Text -> N.Attrib Bool
metaFlag key = lookupMetaOrElse False key <$> lift (gets meta)

whenMeta :: Text -> N.Attrib () -> N.Attrib ()
whenMeta key = whenM (metaFlag key)

insertDebugBorder :: N.Attrib ()
insertDebugBorder = do
  border <- lookupMetaOrElse False "decker.filter.border" <$> lift (gets meta)
  when border $ N.addStyle [("border", "2px solid magenta")]

isCss (key, _) = Text.isPrefixOf "css:" key && Text.length key > 4

imageBlock :: URI -> [Inline] -> N.Attrib Block
imageBlock uri caption = do
  uri <- lift $ transformUri uri ""
  let rendered = URI.render uri
  let fileName = toText $ takeFileName $ toString rendered
  N.takeAttr' ["width", "height"] >>= N.addStyle'
  N.takeId >>= N.set
  N.takeCls [] >>= N.add
  N.takeAttrP isCss >>= (N.addStyle' . Map.mapKeys (Text.drop 4))
  whenMeta "decker.filter.border" $
    N.addStyle [("border", "2px solid magenta")]
  N.dropAttr coreAttribs
  N.takeAttr' ["dir", "xml:lang"] >>= N.addAttr'
  N.takeAttr' [] >>= (N.addAttr' . Map.mapKeys ("data-" <>))
  N.addAttr [("alt", fileName)]
  return $
    Div
      nullAttr
      [ Div
          nullAttr
          ( if null caption
              then [Plain [Image nullAttr [] ("", "")]]
              else
                [ Plain [Image nullAttr [] ("", "")],
                  Div nullAttr [Plain caption]
                ]
          )
      ]
